...
class AdminObjects extends Component {
  constructor(props){
    super(props);
    this.state=({objects:[]});
    this.showEditData=this.showEditData.bind(this);
  }
  ...
  showEditData(id){
    ...
    document.getElementById(id).classList.toggle("showEdit");
  }
      ...
      <div class="tableContainer">
            ...
            <table class="myTable">
                ...
                <tr class="myRow">
                    ...
                    <td><button class="przyciskSzczegoly" onClick ={() => this.showEditData(item.id)}>Edytuj</button></td>
                </tr>,
                <tr class="hidden" id={item.id}>
                <td colSpan="3">{item.openDays}</td>
